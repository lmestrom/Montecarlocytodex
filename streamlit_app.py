...
def simulate_surface_mc(
    ...
    for _ in range(n_experiments):
        ...
        initial_occupied_cells = min(initial_occupied_cells, grid_size * grid_size)
...
def run_stage_surface_growth(
    ...
    CRITICAL FIX: cells/mL -> cells/g MC must include x1000 (because MC is in g/L).
    """
    if mc_g_per_l <= 0 or particles_per_g <= 0:
        raise ValueError("MC g/L and particles/g must be > 0.")

    # ✅ FIXED units:
    # mc_g_per_l is g/L => g/mL = mc_g_per_l / 1000
    # cells_per_g = (cells/mL) / (g/mL) = cells/mL * 1000 / (g/L)
    cells_per_g = (inoc_cells_ml * 1000.0) / mc_g_per_l
    inoc_cells_per_mc_mean = cells_per_g / particles_per_g
...
def run_seed_train(params: dict):
    ...
    # Transfer to BIO200A (volume-based, stays in cells/mL domain)
    ...
    r750E = run_stage_surface_growth("BIO750E", n, d750, MC_BIO750, particles_per_g, max_cells_setpoint, max_cells_sd, inocE, float(params["inoc_sd_cells_per_mc_bio750"]), rng_seed + 2)
    r750F = run_stage_surface_growth("BIO750F", n, d750, MC_BIO750, particles_per_g, max_cells_setpoint, max_cells_sd, inocF, float(params["inoc_sd_cells_per_mc_bio750"]), rng_seed + 3)
    r750G = run_stage_surface_growth("BIO750G", n, d750, MC_BIO750, particles_per_g, max_cells_setpoint, max_cells_sd, inocG, float(params["inoc_sd_cells_per_mc_bio750"]), rng_seed + 4)
    r750H = run_stage_surface_growth("BIO750H", n, d750, MC_BIO750, particles_per_g, max_cells_setpoint, max_cells_sd, inocH, float(params["inoc_sd_cells_per_mc_bio750"]), rng_seed + 5)
    ...
    flat1500A = pd.DataFrame({"time_h": t1500, "total_cells_ml": [inoc_1500A, inoc_1500A], "infectable_cells_ml": [inoc_1500A, inoc_1500A], "multilayer_cells_ml": [0.0, 0.0]})
    flat1500B = pd.DataFrame({"time_h": t1500, "total_cells_ml": [inoc_1500B, inoc_1500B], "infectable_cells_ml": [inoc_1500B, inoc_1500B], "multilayer_cells_ml": [0.0, 0.0]})

    def to_df(stage: StageResult) -> pd.DataFrame:
        return pd.DataFrame({"time_h": stage.t_h, "total_cells_ml": stage.total_cells_ml, "infectable_cells_ml": stage.infectable_cells_ml, "multilayer_cells_ml": stage.multilayer_cells_ml})
    ...
    summary = pd.DataFrame(
        [
            {"Stage": "BIO40", "Inoc_cells/mL": r40.inoc_cells_ml, "End_TOTAL_cells/mL": r40.end_total_cells_ml, "End_INFECTABLE_cells/mL": r40.end_infectable_cells_ml, "Days": d40, "MC_g/L": MC_BIO40},
            {"Stage": "BIO200A", "Inoc_cells/mL": r200.inoc_cells_ml, "End_TOTAL_cells/mL": r200.end_total_cells_ml, "End_INFECTABLE_cells/mL": r200.end_infectable_cells_ml, "Days": d200, "MC_g/L": MC_BIO200A},
            {"Stage": "BIO750E", "Inoc_cells/mL": r750E.inoc_cells_ml, "End_TOTAL_cells/mL": r750E.end_total_cells_ml, "End_INFECTABLE_cells/mL": r750E.end_infectable_cells_ml, "Days": d750, "MC_g/L": MC_BIO750},
            {"Stage": "BIO750F", "Inoc_cells/mL": r750F.inoc_cells_ml, "End_TOTAL_cells/mL": r750F.end_total_cells_ml, "End_INFECTABLE_cells/mL": r750F.end_infectable_cells_ml, "Days": d750, "MC_g/L": MC_BIO750},
            {"Stage": "BIO750G", "Inoc_cells/mL": r750G.inoc_cells_ml, "End_TOTAL_cells/mL": r750G.end_total_cells_ml, "End_INFECTABLE_cells/mL": r750G.end_infectable_cells_ml, "Days": d750, "MC_g/L": MC_BIO750},
            {"Stage": "BIO750H", "Inoc_cells/mL": r750H.inoc_cells_ml, "End_TOTAL_cells/mL": r750H.end_total_cells_ml, "End_INFECTABLE_cells/mL": r750H.end_infectable_cells_ml, "Days": d750, "MC_g/L": MC_BIO750},
            {"Stage": "BIO1500A", "Inoc_cells/mL": inoc_1500A, "End_TOTAL_cells/mL": inoc_1500A, "End_INFECTABLE_cells/mL": inoc_1500A, "Days": 0, "MC_g/L": np.nan},
            {"Stage": "BIO1500B", "Inoc_cells/mL": inoc_1500B, "End_TOTAL_cells/mL": inoc_1500B, "End_INFECTABLE_cells/mL": inoc_1500B, "Days": 0, "MC_g/L": np.nan},
        ]
    )
...
def plot_all_bioreactors_one_plot(series: Dict[str, pd.DataFrame], y_col: str, title: str):
    ...
    for name, df in series.items():
        x = df["time_h_cum"] if "time_h_cum" in df.columns else df["time_h"]
...
def export_zip_csv(series: Dict[str, pd.DataFrame], summary: pd.DataFrame) -> bytes:
    ...
    with zipfile.ZipFile(buffer, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
        ...
        for k, df in series.items():
            zf.writestr(f"{k.replace('/', '_')}.csv", df.to_csv(index=False))
...
st.caption("Option 1 active: Infectable = OCCUPIED + NEWLY_OCCUPIED + INHIBITED (MULTILAYER excluded)")
